...
class OpenAIAdapter(APIAdapter):
    ...
    def _convert_content_blocks(
        ...
        from .message_converters import convert_content_blocks_dispatcher

        return convert_content_blocks_dispatcher(response, format_openai_tool_call)
    ...
    def _convert_messages_to_anthropic(
        ...
        from .message_converters import convert_messages_dispatcher

        return convert_messages_dispatcher(
            openai_messages,
            self._convert_content_to_anthropic,
            self._convert_tool_call_to_anthropic,
        )
    ...
    def _convert_content_to_anthropic(
        ...
        if isinstance(content, str):
            ...
            if re.search(thinking_pattern, content, re.DOTALL):
                return parse_thinking_blocks(content)
            return content
        ...
        for block in content:
            ...
            if hasattr(block, "type"):
                converted = convert_pydantic_block(block)
                if converted:
                    anthropic_content.append(converted)
            elif isinstance(block, dict):
                converted = convert_dict_block(block)
                if converted:
                    anthropic_content.append(converted)
