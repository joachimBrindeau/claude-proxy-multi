...
from claude_code_proxy.auth.api_keys.auth_manager import APIKeyAuthManager
...
def get_api_key_manager(settings: "Settings") -> APIKeyManager | None:
    """Get API key manager if enabled.

    Args:
        settings: Application settings

    Returns:
        APIKeyManager if API keys enabled, None otherwise
...
async def _get_auth_manager_with_settings(
    ...
    if settings.security.api_keys_enabled and credentials and credentials.credentials:
        api_key_manager = get_api_key_manager(settings)
        if api_key_manager:
            api_key = api_key_manager.validate_key(credentials.credentials)
            if api_key:
                # Return a special auth manager for API key users
                return APIKeyAuthManager(api_key)

    # Try bearer token if provided
