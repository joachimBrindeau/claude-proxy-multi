...
class MessageConverter:
    ...
    def _convert_text_block(
        block: sdk_models.TextBlock, mode: SDKMessageMode, pretty_format: bool
    ) -> dict[str, Any]:
        """Convert TextBlock to Anthropic format."""
        text = block.text
        if mode == SDKMessageMode.FORMATTED:
            escaped_text = MessageConverter._escape_content_for_xml(text, pretty_format)
            formatted_text = (
                f"<text>\n{escaped_text}\n</text>\n"
                if pretty_format
    ...
    def _convert_tool_use_block(
        block: sdk_models.ToolUseBlock, mode: SDKMessageMode, pretty_format: bool
    ) -> dict[str, Any] | None:
        """Convert ToolUseBlock to Anthropic format."""
        if mode == SDKMessageMode.FORWARD:
            return block.to_sdk_block()
        if mode == SDKMessageMode.FORMATTED:
    ...
    def _convert_tool_result_block(
        block: sdk_models.ToolResultBlock, mode: SDKMessageMode, pretty_format: bool
    ) -> dict[str, Any] | None:
        """Convert ToolResultBlock to Anthropic format."""
        if mode == SDKMessageMode.FORWARD:
            return block.to_sdk_block()
        if mode == SDKMessageMode.FORMATTED:
    ...
    @staticmethod
    def _convert_thinking_block(
        block: sdk_models.ThinkingBlock, mode: SDKMessageMode, pretty_format: bool
    ) -> dict[str, Any] | None:
        ...
        if mode == SDKMessageMode.FORMATTED:
            ...
            if pretty_format:
                ...
                formatted_text = f"<thinking{signature_attr}>{escaped_text}</thinking>"
            return {"type": "text", "text": formatted_text}
        return None
    ...
    def convert_to_anthropic_response(
        ...
        # Convert content blocks to Anthropic format using dedicated converters
        content_blocks = []
        for block in assistant_message.content:
            converted_block = None
            if isinstance(block, sdk_models.TextBlock):
                converted_block = MessageConverter._convert_text_block(
                    block, mode, pretty_format
                )
            elif isinstance(block, sdk_models.ToolUseBlock):
                converted_block = MessageConverter._convert_tool_use_block(
                    block, mode, pretty_format
                )
            elif isinstance(block, sdk_models.ToolResultBlock):
                converted_block = MessageConverter._convert_tool_result_block(
                    block, mode, pretty_format
                )
            elif isinstance(block, sdk_models.ThinkingBlock):
                converted_block = MessageConverter._convert_thinking_block(
                    block, mode, pretty_format
                )
