...
from claude_code_proxy.claude_sdk.session_client import SessionClient, SessionStatus
from claude_code_proxy.config.claude import SessionPoolSettings
from claude_code_proxy.exceptions import CCProxyError, ServiceUnavailableError
...
class SessionPool:
    ...
    async def _handle_interrupting_session(
        self, session_client: SessionClient, session_id: str, options: ClaudeCodeOptions
    ) -> SessionClient:
        """Handle a session currently being interrupted.
    ...
    async def _handle_stream_timeout(
        ...
        session_id: str,
        is_first_chunk_timeout: bool,
        is_ongoing_timeout: bool,
    ...
    async def _handle_active_stream(
        ...
        """Handle session with active stream that needs cleanup.
        ...
        if session_client.active_stream_handle and (
            ...
            session_client = await self._handle_stream_timeout(
                session_client,
                session_id,
    ...
    async def _get_or_create_session(
        ...
        """Get existing session or create new one with appropriate cleanup.
        ...
        if session_client.status.value == "interrupting":
            return await self._handle_interrupting_session(
                session_client, session_id, options
            )
        ...
        if session_client.has_active_stream or session_client.active_stream_handle:
            session_client = await self._handle_active_stream(
                session_client, session_id, options
            )
