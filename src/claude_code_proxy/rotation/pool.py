...
def _parse_retry_after_date(value: str) -> int | None:
    """Parse retry-after header as HTTP date or ISO8601.
    ...
    try:
        ...
        logger.info("retry_after_parsed_date", date=value, reset_ms=reset_ms)
        return reset_ms
    except (ValueError, TypeError, dateutil_parser.ParserError):
        return None


def _parse_unix_timestamp_header(header_name: str, value: str) -> int | None:
    """Parse header containing Unix timestamp in seconds.
...
def _parse_iso8601_header(header_name: str, value: str) -> int | None:
    """Parse header containing ISO8601 timestamp.
...
def _try_parse_header(
    ...
    """Try to parse a single header with given parser.

    Args:
        headers_lower: Lowercase header map
        header_name: Header name to look up
        parser: Parser function to call
...
def _try_parse_retry_after_header(headers_lower: dict[str, str]) -> int | None:
    ...
    result = _parse_retry_after_seconds(retry_after)
    if result:
        return result

    return _parse_retry_after_date(retry_after)


def _try_parse_unified_headers(headers_lower: dict[str, str]) -> int | None:
    """Try parsing unified rate limit headers.
    ...
    for header_name in (
        ...
        result = _try_parse_header(
            headers_lower, header_name, _parse_unix_timestamp_header
        )
        if result:
            return result
    return None
...
def _try_parse_legacy_headers(headers_lower: dict[str, str]) -> int | None:
    """Try parsing legacy Anthropic rate limit headers.
    ...
    for header_name in (
        ...
        result = _try_parse_header(headers_lower, header_name, _parse_iso8601_header)
        if result:
            return result
    return None
...
def parse_retry_after(headers: dict[str, str]) -> int | None:
    ...
    result = _try_parse_retry_after_header(headers_lower)
    if result:
        return result

    # Try new unified rate limit headers
    result = _try_parse_unified_headers(headers_lower)
    if result:
        return result

    # Fall back to legacy Anthropic headers
    result = _try_parse_legacy_headers(headers_lower)
    if result:
        return result
...
class RotationPool:
    ...
    def __init__(
        ...
        self._rate_limit_repo = RateLimitRepository()
    ...
    async def mark_rate_limited(
        ...
        Persists rate limit to SQLite for restart survival.
        ...
        # Persist rate limit to SQLite for restart survival
        if reset_time is not None:
            reset_dt = datetime.fromtimestamp(reset_time / 1000, tz=UTC)
            await self._rate_limit_repo.mark_limited(
    ...
    async def load_rate_limits_from_db(self) -> int:
        """Load rate limit state from database on startup.

        Restores rate limits that were persisted before a restart.
        Only loads rate limits that are still active (not expired).

        Returns:
            Number of rate limits restored

        """
        import calendar

        limited = await self._rate_limit_repo.get_all_limited()
        restored_count = 0

        for rl in limited:
            if rl.account_name in self._accounts:
                account = self._accounts[rl.account_name]
                # Convert datetime to ms timestamp
                # SQLite returns naive datetimes, but we stored them as UTC
                # Use calendar.timegm to treat the naive datetime as UTC
                resets_at = rl.resets_at
                reset_time_ms = int(calendar.timegm(resets_at.timetuple()) * 1000)
                account.rate_limited_until = reset_time_ms
                account.state = AccountState.RATE_LIMITED
                restored_count += 1
                logger.info(
                    "rate_limit_restored_from_db",
                    account=rl.account_name,
                    resets_at=resets_at.isoformat(),
                )

        if restored_count > 0:
            logger.info(
                "rate_limits_loaded_from_db",
