...
from claude_code_proxy.rotation.accounts import Account
...
class RotationMiddleware:
        ...
        async def send_wrapper(message: Message) -> None:
            if message["type"] == "http.response.start":
                ...
                if status_code == 429:
                    ...
                    await self.pool.mark_rate_limited(
    ...
    def _inject_account_into_scope(
        self, scope: Scope, account: Account, body: bytes
    ) -> None:
        """Inject account information into request scope.
    ...
    async def _send_buffered_429_response(
        self,
        send: Send,
        ...
        """Send a buffered 429 response to client.
    ...
    def _handle_auth_error(
        ...
        if response_status_code in (
            ...
            self.pool.mark_auth_error(
                account_name, error_msg or "Authentication failed"
            )

    def _create_smart_send(
        self,
        send: Send,
        account_name: str,
        attempt: int,
        ...
        """Create a smart send wrapper that handles buffering and retry logic.

        Args:
            send: Original ASGI send callable
            account_name: Account name for rate limit marking
            attempt: Current attempt number
            response_state: Shared state dictionary for tracking response
        ...
        async def smart_send(message: Message) -> None:
            if message["type"] == "http.response.start":
                response_state["status_code"] = message.get(
                    "status", status.HTTP_200_OK
                )
                response_state["headers"] = list(message.get("headers", []))
                ...
                for name, value in response_state["headers"]:
                    if name.lower() == b"content-type":
                        ...
                        break
                response_state["is_streaming"] = "text/event-stream" in content_type

                # For 429 errors, we might retry - don't send yet
                if (
                    response_state["status_code"] == status.HTTP_429_TOO_MANY_REQUESTS
                    and attempt < self.max_retries
                ):
                    response_state["should_retry"] = True
                    return

                # Mark rate limited BEFORE sending response
                if response_state["status_code"] == status.HTTP_429_TOO_MANY_REQUESTS:
                    await self.pool.mark_rate_limited(
                        account_name,
                        headers=_decode_headers(response_state["headers"]),
                    )
                ...
                # If potentially retrying, buffer the response
                if response_state["should_retry"]:
                    if body_chunk:
                        response_state["body_parts"].append(body_chunk)
                    return
    ...
    async def _handle_automatic_rotation(
        ...
        for attempt in range(self.max_retries + 1):
            ...
            account = (
                await self.pool.get_next_available()
                if attempt == 0
                else await self.pool.get_next_available_excluding(tried_accounts)
            )
            ...
            self._inject_account_into_scope(scope, account, body)
            ...
            # Create smart send wrapper
            smart_send = self._create_smart_send(
                send, account.name, attempt, response_state
            )
            ...
            if (
                response_state["status_code"] == status.HTTP_429_TOO_MANY_REQUESTS
                and response_state["should_retry"]
            ):
                logger.info(
                ...
                await self.pool.mark_rate_limited(
                    account.name, headers=_decode_headers(response_state["headers"])
                )
                response_state["body_parts"].clear()
                continue

            # If we got 429 but ran out of retries, send the buffered response
            if (
                response_state["status_code"] == status.HTTP_429_TOO_MANY_REQUESTS
                and not response_state["sent_to_client"]
            ):
                await self._send_buffered_429_response(
                    send,
                    response_state["status_code"],
                    response_state["headers"],
                    response_state["body_parts"],
                    account.name,
                )
