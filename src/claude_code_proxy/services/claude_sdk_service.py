...
from claude_code_proxy.claude_sdk.client import ClaudeSDKClient
...
from claude_code_proxy.claude_sdk.options import OptionsHandler
from claude_code_proxy.claude_sdk.streaming import ClaudeStreamProcessor
...
from claude_code_proxy.models.claude_sdk import SDKMessage, create_sdk_message
from claude_code_proxy.models.messages import MessageResponse
from claude_code_proxy.utils.model_mapping import map_model_to_claude
from claude_code_proxy.utils.simple_request_logger import write_request_log
...
class ClaudeSDKService:
    ...
    def _process_system_message_content(
        self,
        ...
        """Process system message and add to response content.
        ...
        if content_block.get("type") == "system_message":
            ...
            response.content.append(sdk_models.TextBlock.model_validate(content_block))
        else:
            logger.warning(
    ...
    def _add_supplementary_messages(
        self,
        ...
        """Add supplementary message types to response content.

        Args:
            sdk_messages: All SDK messages received
            response: Response to add content to
            mode: SDK message mode
            pretty_format: Whether to use pretty formatting
        """
        ...
        for message in all_messages:
            if isinstance(message, sdk_models.SystemMessage):
                self._process_system_message_content(
                    message, response, mode, pretty_format
                )
    ...
    async def _collect_sdk_messages(
        self, stream_handle: Any
    ) -> tuple[sdk_models.ResultMessage, sdk_models.AssistantMessage, list[Any]]:
        """Collect all messages from SDK stream.

        Args:
            stream_handle: Stream handle from SDK client

        Returns:
            Tuple of (result_message, assistant_message, all_messages)
        ...
        return result_message, assistant_message, sdk_messages
    ...
    async def _complete_non_streaming(
        ...
        (
            ...
            assistant_message,
            sdk_messages,
        ) = await self._collect_sdk_messages(stream_handle)
        ...
        # Add supplementary message types to content
        self._add_supplementary_messages(sdk_messages, response, mode, pretty_format)
    ...
    async def _log_sdk_streaming_chunk(
        ...
        from claude_code_proxy.utils.simple_request_logger import append_streaming_log
