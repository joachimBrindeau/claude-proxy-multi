...
from claude_code_proxy.auth.oauth.constants import (
    OAUTH_AUTHORIZE_URL,
    OAUTH_CLIENT_ID,
    OAUTH_REDIRECT_URI,
    OAUTH_SCOPES,
)
from claude_code_proxy.auth.oauth.token_exchange import OAuthConfig, exchange_code_async
...
# SQLite-backed OAuth flow repository (replaces TTLCache)
_oauth_repo = OAuthFlowRepository()


async def _cleanup_expired_flows() -> int:
    """Remove expired OAuth flows. Returns count removed."""
    count = await _oauth_repo.cleanup_expired()
    if count > 0:
        logger.info("oauth_flows_expired_cleanup", count=count)
    return count


async def _get_pending_account_names() -> list[str]:
    """Get list of account names with pending (non-expired) flows."""
    return await _oauth_repo.get_pending_account_names()
...
async def generate_auth_url(account_name: str) -> tuple[str, str]:
    """Generate OAuth authorization URL and store PKCE verifier."""
    # Cleanup expired flows before creating new one
    await _cleanup_expired_flows()

    code_verifier, code_challenge = generate_pkce()

    # Store OAuth flow state in SQLite (replaces TTLCache)
    await _oauth_repo.create(
        state=code_verifier,
        account_name=account_name,
        code_challenge=code_challenge,
        redirect_uri=OAUTH_REDIRECT_URI,
        ttl_seconds=OAUTH_FLOW_TTL_SECONDS,
    )
...
async def start_oauth_redirect(request: Request, account_name: str) -> Response:
    ...
    auth_url, state = await generate_auth_url(name)
...
async def oauth_return(request: Request, state: str = "") -> HTMLResponse:
    ...
    flow = await _oauth_repo.get_valid(state)
    if not flow:
        return render_page(
...
async def _validate_oauth_flow_state(
    state: str,
) -> tuple[OAuthFlow | None, str | None]:
    ...
    flow = await _oauth_repo.get_valid(state)
    if not flow:
        pending_accounts = await _get_pending_account_names()
        logger.warning(
...
async def complete_oauth(
    ...
    flow, error_msg = await _validate_oauth_flow_state(state)
    if error_msg:
    ...
    try:
        ...
        await _oauth_repo.delete(state)
...
async def cancel_oauth(
    ...
    # Get flow info before deleting for logging
    flow = await _oauth_repo.get_valid(state)
    deleted = await _oauth_repo.delete(state)
        ...
        if capacity.error:
            ...
            if "rate limited" in capacity.error.lower():
                ...
                if account.state != "rate_limited":
                    ...
                    await pool.mark_rate_limited(account_name)
            return render_page(
                request,
