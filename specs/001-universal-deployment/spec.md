...
- Q: How should the installer endpoint at https://get.claude-code-proxy.dev be hosted and managed? → A: GitHub Pages with custom domain (get.claude-code-proxy.dev CNAME to joachimbrindeau.github.io/claude-code-proxy/install.sh) - zero cost, automatic deployment via GitHub Actions
...
1. **Given** a developer has Docker installed, **When** they run `curl -fsSL https://get.claude-code-proxy.dev | sh`, **Then** the installer detects Docker, downloads the compose file, starts the service, and opens their browser to the accounts page
...
- **FR-021**: GitHub Actions MUST deploy the installer script to GitHub Pages automatically on commits to main branch, making it accessible at https://get.claude-code-proxy.dev via CNAME configuration
