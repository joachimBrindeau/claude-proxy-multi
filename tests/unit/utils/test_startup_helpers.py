    ...
    async def test_authentication_validation_error(
        ...
        with patch(
            ...
            with patch("claude_code_proxy.utils.startup_helpers.logger") as mock_logger:
                ...
                # Verify error was logged (using exception, not error)
                mock_logger.exception.assert_called_once()
                call_args = mock_logger.exception.call_args[1]
                assert (
                    "claude_token_validation_error"
                    in mock_logger.exception.call_args[0]
                )
    ...
    async def test_claude_cli_check_error(
        ...
        with patch(
            ...
            with patch("claude_code_proxy.utils.startup_helpers.logger") as mock_logger:
                ...
                # Verify error log was called (using exception, not error)
                mock_logger.exception.assert_called_once()
                call_args = mock_logger.exception.call_args[1]
                assert "claude_cli_check_failed" in mock_logger.exception.call_args[0]
    ...
    async def test_scheduler_shutdown_error(self, mock_app: FastAPI) -> None:
        ...
        with patch(
            ...
            with patch("claude_code_proxy.utils.startup_helpers.logger") as mock_logger:
                ...
                mock_logger.exception.assert_called_once()
                call_args = mock_logger.exception.call_args[1]
                assert "scheduler_stop_failed" in mock_logger.exception.call_args[0]
    ...
    async def test_flush_streaming_batches_error(self, mock_app: FastAPI) -> None:
        ...
        with patch(
            ...
            with patch("claude_code_proxy.utils.startup_helpers.logger") as mock_logger:
                ...
                call_args = mock_logger.exception.call_args[1]
                assert (
                    "streaming_batches_flush_failed"
                    in mock_logger.exception.call_args[0]
                )
    ...
    async def test_claude_detection_startup_error_with_fallback(
        ...
        with patch(
            ...
            with patch("claude_code_proxy.utils.startup_helpers.logger") as mock_logger:
                ...
                call_args = mock_logger.exception.call_args[1]
                assert (
                    "claude_detection_startup_failed"
                    in mock_logger.exception.call_args[0]
                )
