...
class PortSpecParts(TypedDict):
    """Type-safe components of a parsed port specification."""
...
def _parse_ipv6_port_spec(
    ...
    Args:
        port_part: The port part without protocol suffix
        port_spec: Original port specification for error messages

    Returns:
        PortSpecParts with host_ip, host_port, container_port
...
def _validate_port_numbers(
    ...
    Args:
        ...
        container_port: Container port string
        port_spec: Original port specification for error messages
...
def _validate_ip_address(host_ip: str, port_spec: str) -> None:
...
def validate_port_spec(port_spec: str) -> str:
    ...
    if port_part.startswith("["):
        parsed = _parse_ipv6_port_spec(port_part, port_spec)
        _validate_ip_address(parsed["host_ip"], port_spec)
        _validate_port_numbers(parsed["host_port"], parsed["container_port"], port_spec)
    else:
        ...
        if len(parts) == 2:
            ...
            _validate_port_numbers(host_port, container_port, port_spec)
        elif len(parts) == 3:
            # Format: "host_ip:host_port:container_port"
            host_ip, host_port, container_port = parts
            _validate_ip_address(host_ip, port_spec)
            _validate_port_numbers(host_port, container_port, port_spec)
        else:
            raise create_docker_error(
